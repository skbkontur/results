...
namespace Kontur.Tests.Results.Conversion.Combinations.Then.ResultFailureResultPlain
{
    [TestFixture]
    internal class Method_Should
    {
        private static IEnumerable<(ResultFailure<string> Result1, Result<string> Result2, ResultFailure<string> Result)> CreateCases()
        {
            var result = Result.Fail("fault");

            yield return (result, Result<string>.Succeed(), result);
            yield return (result, Result<string>.Fail("unused"), result);
        }

        private static readonly Func<ResultFailure<string>, Func<Result<string>>, ValueTask<ResultFailure<string>>>[] FactoryMethodsValueTasks =
        {
        ...
        private static readonly Func<ResultFailure<string>, Func<Result<string>>, Task<ResultFailure<string>>>[] FactoryMethodsTasks =
        {
        ...
        private static IEnumerable<Func<ResultFailure<string>, Func<Result<string>>, Task<ResultFailure<string>>>> FactoryMethods()
        {
        ...
        private static readonly Func<ResultFailure<string>, Result<string>, ValueTask<ResultFailure<string>>>[] PlainMethodsValueTasks =
        {
        ...
        private static readonly Func<ResultFailure<string>, Result<string>, Task<ResultFailure<string>>>[] PlainMethodsTasks =
        {
        ...
        private static IEnumerable<Func<ResultFailure<string>, Result<string>, Task<ResultFailure<string>>>> AllMethods()
        {
        ...
        public async Task Do_Not_Call_Delegate_If_Failure(Func<ResultFailure<string>, Func<Result<string>>, Task<ResultFailure<string>>> then)
        {
            var result = Result.Fail("fault");
