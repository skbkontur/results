...
namespace Kontur.Tests.Results.Conversion.Combinations.Then.ResultPlainResultFailure
{
    [TestFixture]
    internal class Method_Should
    {
        private static IEnumerable<(Result<string> Result1, ResultFailure<string> Result2, ResultFailure<string> Result)> CreateCases()
        {
            var result = Result.Fail("bar");
            yield return (Result<string>.Succeed(), result, result);

            const string fault = "fault";
            yield return (Result<string>.Fail(fault), Result.Fail("unused"), Result.Fail(fault));
        }

        private static readonly Func<Result<string>, Func<ResultFailure<string>>, ValueTask<ResultFailure<string>>>[] FactoryMethodsValueTasks =
        {
        ...
        private static readonly Func<Result<string>, Func<ResultFailure<string>>, Task<ResultFailure<string>>>[] FactoryMethodsTasks =
        {
        ...
        private static IEnumerable<Func<Result<string>, Func<ResultFailure<string>>, Task<ResultFailure<string>>>> FactoryMethods()
        {
        ...
        private static readonly Func<Result<string>, ResultFailure<string>, ValueTask<ResultFailure<string>>>[] PlainMethodsValueTasks =
        {
        ...
        private static readonly Func<Result<string>, ResultFailure<string>, Task<ResultFailure<string>>>[] PlainMethodsTasks =
        {
        ...
        private static IEnumerable<Func<Result<string>, ResultFailure<string>, Task<ResultFailure<string>>>> AllMethods()
        {
        ...
        public async Task Do_Not_Call_Delegate_If_Failure(Func<Result<string>, Func<ResultFailure<string>>, Task<ResultFailure<string>>> then)
        {
            var result = Result<string>.Fail("unused");
