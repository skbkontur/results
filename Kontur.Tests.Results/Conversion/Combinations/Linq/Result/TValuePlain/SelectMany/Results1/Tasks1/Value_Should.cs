...
namespace Kontur.Tests.Results.Conversion.Combinations.Linq.Result.TValuePlain.SelectMany.Results1.Tasks1
{
        ...
        where TFixtureCase : IFixtureCase, new()
    {
        ...
        public Task<Result<string>> Result_Task(Result<string, int> result)
        {
            return
                from x in result
                from y in Task1000
        ...
        [TestCaseSource(nameof(Cases))]
        public Task<Result<string>> TaskResult_Task(Result<string, int> result)
        {
            return
